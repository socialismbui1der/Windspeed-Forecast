...
# --- 配置常量 ---
ATTRIBUTES = ['TEM', 'RHU', 'PRS', 'WINS', 'WIND']
RAW_FEAT_DIM = len(ATTRIBUTES)
FEAT_DIM = 6
# 假设你的数据文件名和路径
...
def get_all_data_and_indices(dir_path, eval_length, train_ratio=0.7, valid_ratio=0.1):
    ...
    for sid in station_ids:
        ...
        for attr in ATTRIBUTES:
            ...
            mask = (~series.isna()).astype(np.float32).values.reshape(T, 1)
            
            # --- 特殊处理风向 (WIND) ---
            if attr == 'WIND': # 假设风向特征名叫 'WIND'
                # 1. 填充 NaN 以便计算 sin/cos (mask 记录了它是缺失的，所以填 0 没关系)
                #    但在归一化前最好保持原始值，这里先暂存
                wd_values = np.nan_to_num(values, nan=0.0)
                
                # 2. 角度转弧度
                wd_rad = np.radians(wd_values)
                
                # 3. 计算 sin 和 cos
                wd_sin = np.sin(wd_rad)
                wd_cos = np.cos(wd_rad)
                
                # 4. 如果原始风向是 NaN，那么 sin/cos 也应该是 NaN (或者 0，配合 mask)
                #    为了保持 mask 逻辑一致，我们让 sin/cos 的 mask = 原始风向 mask
                #    值填充为 0 (之后会被 mask 盖掉)
                wd_sin[mask == 0] = 0.0
                wd_cos[mask == 0] = 0.0
                
                # 添加到列表 (注意顺序：TEM, RHU, PRS, WINS, SIN, COS)
                station_feat_list.append(wd_sin)
                station_feat_list.append(wd_cos)
                
                # Mask 也要重复两次 (因为一个风向变成了两个特征)
                station_mask_list.append(mask)
                station_mask_list.append(mask)
                
            else:
                # 非风向特征，直接添加
                station_feat_list.append(values)
                station_mask_list.append(mask)

        # 拼接该站点的所有特征
        # 结果形状: (T, 6) -> [TEM, RHU, PRS, WINS, SIN, COS]
    ...
    # 4. 沿特征维拼接所有站点 (保持不变)
    full_data_with_nan = np.concatenate(full_data_list, axis=1) # (T, N * 6)
...
def calculate_train_mean_std(full_data_with_nan, full_c_mask, train_indices, eval_length):
    ...
    for k in range(FEAT_DIM):
        ...
        # 针对 sin/cos 风向特征 (索引 4 和 5)，我们通常不需要归一化 (Z-score)
        # 因为它们本身就是归一化的 (-1 到 1)
        if k >= 4: # 假设最后两个是风向
            mean[k] = 0.0
            std[k] = 1.0 
...
def get_forecast_dataloader(batch_size, device, eval_length=36):
    ...
    train_ratio = 0.8   # 只保留原 test 的 10%
    new_train_size = max(1, int(len(train_indices) * train_ratio))
    train_indices = train_indices[:new_train_size]

    val_ratio = 0.4   # 只保留原 test 的 10%
    new_val_size = max(1, int(len(valid_indices) * val_ratio))
    valid_indices = valid_indices[:new_val_size]

    test_ratio = 0.5   # 只保留原 test 的 10%
    new_test_size = max(1, int(len(test_indices) * test_ratio))
    test_indices = test_indices[:new_test_size]
