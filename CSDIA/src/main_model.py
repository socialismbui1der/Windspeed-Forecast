...
class CSDI_base(nn.Module):
    def __init__(self, target_dim, config, device):
        ...
        config_diff["side_dim"] = self.emb_total_dim + config_graph["station_emb_dim"]
        ...
        self.D_station = config_graph["station_emb_dim"]   # 例如 16 或 32
        self.station_emb = nn.Parameter(
            torch.randn(self.num_stations, self.D_station) * 0.02
        )
        self.feature_dim = 4
    ...
    def evaluate(self, batch, n_samples):
        ...
        with torch.no_grad():
            ...
            side_info = self.get_side_info(observed_tp_eff, cond_mask_eff)  # (B_eff, C_base, K, L)
...
class CSDI_Forecasting(CSDI_base):
    ...
    def get_side_info(self, observed_tp, cond_mask):
        """
        输出 side_info = 时间嵌入 + 特征嵌入 + 静态站点嵌入 + cond_mask
        ----------
        observed_data: (B, N, K, L)
        cond_mask:     (B, N, K, L)
        observed_tp:   (B, L)
        ...
        # ============================================================
        # 1. 时间嵌入 (B, D_time, N, K, L)
        # ============================================================
        # 原始: (B, L, D_time)
        time_embed = self.time_embedding(observed_tp, self.emb_time_dim)

        # 扩展维度
        time_embed = time_embed[:, None, None, :, :]   # (B,1,1,L,D_time)
        time_embed = time_embed.expand(B, N, K, L, self.emb_time_dim)
        time_embed = time_embed.permute(0, 4, 1, 2, 3) # (B,D_time,N,K,L)

        # ============================================================
        # 2. 特征嵌入 (B, D_feature, N, K, L)
        # ============================================================
        feature_ids = torch.arange(self.target_dim_base, device=device)
        feature_embed = self.embed_layer(feature_ids)  # (K, D_feature)

        feature_embed = feature_embed.view(1, 1, K, 1, -1)
        feature_embed = feature_embed.expand(B, N, K, L, -1)
        feature_embed = feature_embed.permute(0, 4, 1, 2, 3)  # (B,D_feature,N,K,L)

        # ============================================================
        # 3. 静态站点嵌入 (B, D_station, N, K, L)
        # ============================================================
        # station_emb: (N, D_station)
        station_emb = self.station_emb.t()  # → (D_station, N)

        # 添加维度为 (1, D_station, N, 1, 1)
        station_embed = station_emb.view(1, self.D_station, N, 1, 1)

        # broadcast 到 (B, D_station, N, K, L)
        station_embed = station_embed.expand(B, -1, N, K, L)

        # ============================================================
        # 4. cond_mask (B, 1, N, K, L)
        # ============================================================
        ...
        side_info = torch.cat(
            [
                time_embed,       # (B, D_time, N, K, L)
                feature_embed,    # (B, D_feature, N, K, L)
                station_embed,    # (B, D_station, N, K, L)
                cond_mask         # (B, 1, N, K, L)
            ],
            dim=1
        )
    ...
    def forward(self, batch, is_train=1):
        ...
        loss = loss_func(
            obs_data,     # (B_eff, F_raw, L)，标准化后的“真实空间”
            cond_mask,    # (B_eff, F_raw, L)
            obs_mask,     # (B_eff, F_raw, L)
            side_info,        # (B_eff, *, F_raw, L)，包含 GCN 辅助信息
            is_train,
        )
        return loss
