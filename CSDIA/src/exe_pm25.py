...
path = "/workspace/CSDIA/config/" + args.config
...
forecast_model = CSDI_Forecasting(config, args.device,4).to(args.device)


foldername = r"/workspace/CSDIA/save"
pure_evaluate = True
...
if not pure_evaluate:
    ...
    forecast_model.load_state_dict(torch.load("/workspace/CSDIA/save/model.pth"))
